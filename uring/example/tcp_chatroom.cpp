...
struct session {
  ...
  session(connection<ipv4, tcp> con) : con(con) {
    ...
    co_await [&]() -> awaitable<void> {
      while (1) {
        ...
        for (int i = 0; i < n1; i++) {
          *message += buffer[i];
          if (buffer[i] == '\n') {
            if (*co_await handle(std::exchange(message, make_unique<string>()))){
              co_return;
            }
            continue;
          }
  ...
  awaitable<bool> handle(shared_ptr<string> message) {
    if (message->starts_with("/help")) {
      ...
      co_await send(merge_message(system_name,make_shared<string>("Online User:\n")));
      for (auto name:names){
        co_await send(merge_message(system_name,make_shared<string>(name->substr(0,name->size()-3)+"\n")));
      }
